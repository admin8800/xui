...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/admin8800/xui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/admin8800/xui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_usage() {
    ...
    echo "x-ui enable       - 启用 x-ui 开机自启"
    echo "x-ui disable      - 禁用 x-ui 开机自启"
